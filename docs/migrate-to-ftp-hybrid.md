...
此命令会明确提示并只在 `.git-deploy/ftp-probe/<随机 ID>` 创建、读取、Rename、删除临时文件，完成后清理本次 Probe Root。服务器必须证明大小写敏感路径（两个大小写变体可同时存在、独立回读/删除、case-only rename）、MLSD 类型、二进制零/非零文件回读、跨目录 Rename、Rename Replace、DELE 与 RMD。Profile 缺失、损坏、Target/Banner 变化、Schema 1 或运行期能力错误时重新执行；`--probe-ftp-hybrid` 本身会明确覆盖 Profile，不再需要 `--reprobe`。不要改用 LIST/NLST 或关闭校验。
...
FTP backend 总是先发布当前文件，再删除受管孤儿；Ownership 最后提交，Local State 在 Ownership 之后保存。`PREPARED`、`FILES_PUBLISHED`、`PRUNED` 中断后继续运行普通 `--remote-plan` 和部署；Marker 会要求 Project、Mapping、Remote、Target Fingerprint、HEAD、Local Manifest 与阶段对应的 Ownership Hash 匹配。进入 `OWNERSHIP_COMMITTED` 或 `STATE_COMPLETE` 后运行 `git-deploy prod --recover`：该路径不 Build、不读取当前 State、不扫描当前 Hybrid，使用 Marker 中冻结的 State 完成收口，因此当前 HEAD 或 Build 已变化也不影响旧部署完成。
...
- 本地和远端每个目录都拒绝 NFC + casefold 冲突名称；Windows/IIS 或大小写语义不明的服务器会在 Probe 阶段 Fail Closed。
