...
| B03E | 接通 Docker BuildRunner 成功与非零退出路径 | Docker build backend、runner facade、`tests/test_docker_build_runner.py` | `uv run pytest tests/test_docker_build_runner.py -q -k 'lifecycle_success or lifecycle_nonzero'` 通过；使用不可变 image ID 和固定命名容器运行，成功采集产物、非零退出返回结构化错误，均不回退 host且远端调用为 0 | B03, B03D | 待办 |
| B03ET | 实现 Docker timeout/Ctrl-C 协调终止 | Docker lifecycle、`tests/test_docker_build_runner.py` | `uv run pytest tests/test_docker_build_runner.py -q -k 'timeout or interrupt'` 通过；timeout/Ctrl-C 均按 stop→限时等待→kill→remove，保留脱敏阶段错误且不会启动远端 mutation | B03E | 待办 |
| B03EC | 实现 Docker UID/GID 与清理失败门禁 | Docker lifecycle、`tests/test_docker_build_runner.py` | `uv run pytest tests/test_docker_build_runner.py -q -k 'ownership or cleanup'` 通过；产物归属当前 UID/GID，成功/失败/中断零残留；remove 失败保留不含 secret 的容器 ID 并阻断远端，后续显式清理可幂等重试 | B03E | 待办 |
...
| B03OD | 将 `op run` 环境变量接入 Docker runner | secret runner、Docker backend、`tests/test_onepassword_runner.py` | `uv run pytest tests/test_onepassword_runner.py -q -k docker_env` 通过；调用链为 op→受限 docker，强制 digest；Docker argv 只含 `--env NAME`，client/container 无 `OP_*`，构建变量只进入声明名称；摘要明确 daemon/admin 可读取存活容器 metadata | B03O, B03ET, B03EC | 待办 |
| B03OX | 实现 Docker secret metadata 输出脱敏与清理门禁 | secret runner、Docker lifecycle、`tests/test_onepassword_runner.py` | `uv run pytest tests/test_onepassword_runner.py -q -k docker_metadata` 通过；fake inspect 返回 sentinel 时 git-deploy 不记录/渲染原文，成功/失败/中断删除命名容器；只断言自身输出无泄漏，不断言 daemon metadata 无 env value | B03OD | 待办 |
...
| B05D | 实现 Docker build fingerprint conformance | Docker backend、build cache、`tests/test_build_cache.py` | `uv run pytest tests/test_build_cache.py -q -k docker` 通过；不可变 image ID、platform、network、pull policy、UID/GID 任一变化 miss；不改变 physical target/managed policy | B05, B03EC | 待办 |
| B05O | 实现 1Password cache-bypass conformance | secret runner、build cache、`tests/test_build_cache.py` | `uv run pytest tests/test_build_cache.py -q -k onepassword` 通过；启用 1Password 时固定 bypass cache，state 只含 provider/变量名/不透明摘要，不含 reference URI/值；secret rotation 必定重新构建 | B05, B03OX | 待办 |
| B06 | 以可信 current artifact manifest 生成 target 差量 | artifact planner、state schema、`tests/test_artifact_planner.py` | `uv run pytest tests/test_artifact_planner.py -q -k trusted_diff` 通过；已有可信 manifest 时覆盖新增、修改、删除、mode、owner/content ref；manifest 缺失时返回 `baseline_required`，不得读取远端 bytes 建立 current | B05, S08 | 待办 |
| B06A | 实现 artifact 首次可信 baseline 计划 | artifact planner/baseline builder、`tests/test_artifact_planner.py` | `uv run pytest tests/test_artifact_planner.py -q -k baseline` 通过；仅允许从已知 source revision 构建 baseline，或显式 empty 并只读验证全部 destination 不存在；未知/部分存在/构建不可复现均阻断且 state/远端写为 0 | B06 | 待办 |
| B07 | 合并 source/artifact 计划并执行 owner 冲突门禁 | combined planner、`tests/test_combined_planner.py` | `uv run pytest tests/test_combined_planner.py -q` 通过，统一 current/target file state；source/artifact、artifact/artifact 目标路径冲突在远端连接前失败 | P05, B06A | 待办 |
| B08 | 实现逐文件 content provider 与磁盘 spool | planner/executor/content provider、`tests/test_streaming.py` | `uv run pytest tests/test_streaming.py -q -k 'provider or spool'` 通过；小型 fixture 逐文件/chunk 读取，spool 权限受控，成功/读取失败/消费中断后临时文件清理，不把全量目标 bytes 聚合到 plan | B07 | 待办 |
| B08P | 增加 500 MB streaming 资源预算门禁 | streaming performance fixture、`tests/test_streaming.py` | `uv run pytest tests/test_streaming.py -q -k resource_budget` 通过；固定 500 MB 多文件 fixture 的额外峰值 RSS、单 chunk、spool 磁盘均不超过测试常量，成功/失败后临时占用归零 | B08 | 待办 |
| B09 | 将 source/artifact 纳入同一远端事务与 state 迁移 | executor、rollback、`tests/test_combined_transaction.py` | `uv run pytest tests/test_combined_transaction.py -q` 通过，任一 source/artifact 上传、删除、hook、verify 失败都恢复统一 before bytes 与 before state | B08P, D09C, R01 | 待办 |
...
| I02D | 增加真实 Docker scratch image 成功门禁 | 本地 scratch fixture image、`tests/integration/test_docker_build.py` | `uv run pytest tests/integration/test_docker_build.py -q -k success` 通过；从本地静态 Go helper 构建 scratch image且不访问 registry，覆盖固定 mount、UID/GID、成功 artifact hash 和容器删除 | I02, B00D | 待办 |
| I02DF | 增加真实 Docker 失败/超时/中断门禁 | Docker lifecycle integration、`tests/integration/test_docker_build.py` | `uv run pytest tests/integration/test_docker_build.py -q -k 'nonzero or timeout or interrupt'` 通过；真实 daemon 覆盖非零、超时、Ctrl-C 的 stop/kill/remove 顺序，均不连接远端 | I02D, B03ET | 待办 |
| I02DC | 增加真实 Docker 零残留与 secret 边界门禁 | Docker daemon inspection、`tests/integration/test_docker_build.py` | `uv run pytest tests/integration/test_docker_build.py -q -k 'cleanup or secret_boundary'` 通过；各路径结束后无 fixture 容器；测试明确证明存活容器 Config.Env 可见哨兵、git-deploy 输出不含哨兵且删除后不可 inspect，不把 daemon 视为保密边界 | I02D, B03OX | 待办 |
...
| I04C | 编写 host/Docker/1Password 构建安全文档 | `README.md`、构建文档 | 文档覆盖 project/remote override、host 用户权限、Docker image/digest/network 信任、daemon/admin 可读取存活容器 env metadata、容器删除、op service account、cache bypass、dry-run 零调用和 artifact 无 DLP 保证；不含真实 reference/token/value | B10O, I02DC | 待办 |
